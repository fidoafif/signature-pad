...
class SignaturePad extends Component<PropsType> {
  ...
  _onNavigationChange = (args: ?object) => {
    this._parseMessageFromWebViewNavigationChange(unescape(args.url));
  };

  _parseMessageFromWebViewNavigationChange = (newUrl: ?string) => {
    //Example input:
  ...
  _attemptToExecuteNativeFunctionFromWebViewMessage = (message: object): boolean => {
  ...
  _bridged_jsError = (args: ?string) => {
    this.props.onError({details: args});
  };

  _bridged_finishedStroke = (img: ?object) => {
    this.props.onChange(img.base64DataUrl);
    this.setState({img});
  };

  _renderError = (args: ?string) => {
    this.props.onError({details: args});
  };
